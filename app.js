...
const additionalQuestionSeeds = {
  leadership: [
    "周囲が遠慮している場面でも、必要なら最初に意見を出せる",
    "新しい企画では、目的やゴールを自分なりに整理して提案したい",
    "人任せにせず、自分ができる役割を探すことが多い",
    "うまく進んでいない場面では、現状を変える行動を取りたい",
    "チームの課題に気づいたら、改善のきっかけを作ろうとする",
    "自分の発言が周囲に影響することを意識している",
    "責任ある役割を任されると、期待に応えたい気持ちが強くなる",
    "成果を出すために、関係者へ自分から働きかけることがある",
    "指示を待つだけでなく、次に必要なことを考えて動く"
  ],
  collaboration: [
    "相手が何に困っているかを想像して行動することがある",
    "グループ内の情報共有が不足していると気になる",
    "自分の成果だけでなく、周囲が動きやすいかも大切にする",
    "意見が対立したとき、相手を否定せず整理して話したい",
    "一緒に働く人の負担が偏らないよう配慮する",
    "チームで決めた方針には、責任を持って協力したい",
    "相手の得意不得意に合わせて頼み方を変えることがある",
    "周囲の協力があって成果が出ると感じることが多い",
    "自分と違う考え方にも学べる点があると思う"
  ],
  planning: [
    "複数の作業があるとき、先に順番を決めてから進める",
    "期限が先でも、早めに少しずつ取りかかるほうだ",
    "大きな目標は、小さな行動に分けると進めやすい",
    "作業後に、次はもっと効率よく進める方法を考える",
    "予定を立てると、やるべきことが明確になって安心する",
    "継続が必要なことは、習慣化できる仕組みを作りたい",
    "確認不足によるミスを防ぐため、見直し時間を確保する",
    "優先順位を決めるとき、重要度と期限の両方を見る"
  ],
  adaptability: [
    "想定外の変更があっても、まず代替案を考えようとする",
    "新しいやり方を試すことで、効率が上がるなら取り入れたい",
    "環境が変わったとき、早く慣れるために周囲を観察する",
    "過去のやり方が合わなければ、別の方法に切り替えられる",
    "初めての仕事でも、調べながら進めることに抵抗が少ない",
    "相手や場面によって、自分の伝え方を調整する",
    "変化が多い状況でも、学ぶことがあると前向きに考えたい",
    "失敗した方法にこだわらず、次の試し方を考える"
  ],
  analysis: [
    "結論を出す前に、前提条件が正しいか確認したい",
    "問題が起きたとき、感情と事実を分けて考える",
    "複雑な話は、要素に分けると理解しやすい",
    "数字や根拠があると、判断に納得しやすい",
    "説明するときは、相手が追いやすい順番を意識する",
    "原因が一つとは限らないと考え、複数の可能性を見る",
    "情報が足りないときは、追加で確認してから判断したい",
    "改善案を考えるとき、効果と手間の両方を比べる"
  ],
  resilience: [
    "忙しい時期でも、まず落ち着いて状況を整理したい",
    "うまくいかない経験があっても、次に活かす方法を考える",
    "緊張する場面では、事前準備で不安を減らす",
    "厳しい指摘を受けても、必要な改善点を拾おうとする",
    "負荷が高いときは、抱え込まず早めに相談したい",
    "疲れを感じたら、集中力を戻すための休憩を取る",
    "予定外の問題が起きても、できることから手をつける",
    "困難な状況でも、最後まで投げ出さないよう意識する"
  ]
};

const combinedQuestionSeeds = Object.fromEntries(
  Object.entries(questionSeeds).map(([trait, texts]) => [
    trait,
    [...texts, ...(additionalQuestionSeeds[trait] || [])]
  ])
);

const baseQuestions = Object.entries(combinedQuestionSeeds).flatMap(([trait, texts]) =>
